using Oceananigans, Random, Printf, JLD, OceananigansAnalysis, Statistics, PyPlot
...
outputwriter = JLDOutputWriter(dir="../test", prefix=prefix, frequency=test_steps)
...
u₁ = FaceFieldX(u₁, model.grid)
w₁ = FaceFieldZ(w₁, model.grid)
T₁ = CellField(T₁, model.grid)

u_diff = FaceFieldX(data(model.velocities.u) .- data(u₁), model.grid)
w_diff = FaceFieldZ(data(model.velocities.w) .- data(w₁), model.grid)
T_diff = CellField(data(model.tracers.T) .- data(T₁), model.grid)
...
@show maximum(data(w_diff))
@show minimum(data(w_diff))
